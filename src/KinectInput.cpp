...
void KinectInput::glutIdle()
{
	glutPostRedisplay();
}
void KinectInput::glutDisplay()
{
	KinectInput::ms_self->Display();
}
void KinectInput::glutKeyboard(unsigned char key, int x, int y)
{
	KinectInput::ms_self->OnKey(key, x, y);
}

KinectInput::KinectInput(const char* strSampleName) : m_poseUser(0)
{
	ms_self = this;
...
openni::Status KinectInput::Init(int argc, char **argv)
{
	m_pTexMap = NULL;
  ...
  // MARK: OSC (Test During Setup)
//  char buffer[OUTPUT_BUFFER_SIZE];
//  osc::OutboundPacketStream p( buffer, OUTPUT_BUFFER_SIZE );
//  
//  p << osc::BeginBundleImmediate
//  << osc::BeginMessage( "/test1" )
//  << "Opening OSC connection" << osc::EndMessage
//  << osc::EndBundle;
//  
...
openni::Status KinectInput::Run()	//Does not return
{
	glutMainLoop();
...
void KinectInput::Display()
{
	...
	for (int i = 0; i < users.getSize(); ++i)
	{
        ...
        // MARK: Send Data.
        SendSkeleton(m_pUserTracker, user);
        SendCenterOfMass(m_pUserTracker, user);
        SendBoundingBox(m_pUserTracker, user);
			}
		}
...
void KinectInput::OnKey(unsigned char key, int /*x*/, int /*y*/)
{
	switch (key)
	{
    case 27:
...
openni::Status KinectInput::InitOpenGL(int argc, char **argv)
{
	glutInit(&argc, argv);
...
void KinectInput::InitOpenGLHooks()
{
...
// MARK: Send OSC For a user.
void KinectInput::SendSkeleton(nite::UserTracker* pUserTracker, const nite::UserData& userData) {
  
  for (nite::JointType joint = nite::JOINT_HEAD; joint < nite::JOINT_RIGHT_FOOT; joint++) {
    SendLimb(userData.getSkeleton().getJoint(joint), userData.getId(), (int) joint);
  }
  
}

void KinectInput::SendLimb(const nite::SkeletonJoint& joint, int user_id, int joint_id) {
  
  char buffer[OUTPUT_BUFFER_SIZE];
//  osc::OutboundPacketStream p( buffer, OUTPUT_BUFFER_SIZE );
//  
//  p << osc::BeginBundleImmediate
//  << osc::BeginMessage( "/user/joint/coordinates" ) << user_id
//  << joint_id
...
void KinectInput::SendCenterOfMass(nite::UserTracker* pUserTracker, const nite::UserData& userData) {
  
//  char buffer[OUTPUT_BUFFER_SIZE];
//  osc::OutboundPacketStream p( buffer, OUTPUT_BUFFER_SIZE );
//  
//  p << osc::BeginBundleImmediate
//  << osc::BeginMessage( "/user/centerofmass" ) << userData.getId()
//  << userData.getCenterOfMass().x << userData.getCenterOfMass().y << userData.getCenterOfMass().z
//  << osc::EndMessage
//  << osc::EndBundle;
//  
//  transmitSocket.Send( p.Data(), p.Size() );
...
void KinectInput::SendBoundingBox(nite::UserTracker* pUserTracker, const nite::UserData& userData) {
  
//  char buffer[OUTPUT_BUFFER_SIZE];
//  osc::OutboundPacketStream p( buffer, OUTPUT_BUFFER_SIZE );
//  
//  p << osc::BeginBundleImmediate
//  << osc::BeginMessage( "/user/boundingbox" ) << userData.getId()
//  << userData.getBoundingBox().min.x << userData.getBoundingBox().min.y << userData.getBoundingBox().min.z
//  << userData.getBoundingBox().max.x << userData.getBoundingBox().max.y << userData.getBoundingBox().max.z
//  << osc::EndMessage
//  << osc::EndBundle;
//  
//  transmitSocket.Send( p.Data(), p.Size() );
